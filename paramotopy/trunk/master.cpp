...
void master(std::string filename,
  ...
  for (int i = 1; i < numprocs; ++i){
    int biggestnumrecv;
    // receive info that initial solve for MPI_ANY_SOURCE is complete
	  MPI_Recv(&biggestnumrecv, 1, MPI_INT, MPI_ANY_SOURCE, 16, MPI_COMM_WORLD, &status);
	  if (biggestnumrecv > biggestnumsent){
	    biggestnumsent = biggestnumrecv;
	  }
	  

	  
  } 
  // end for -- reset biggestnumsent ? -- this was initial step2 solve -- to get num.out
  // in the proper format for the bertini SLP
