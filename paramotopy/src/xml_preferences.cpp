...
void ProgSettings::set_path_failure_settings_from_steptwo(){
  ...
  for (iter=settings["stage2bertini"].begin(); iter != settings["stage2bertini"].end(); iter++){
    std::string setting_name = (*iter).first;
    switch (settings["stage2bertini"][setting_name].type) {
    case 0:
      setValue("PathFailureBertiniBase",setting_name,settings["stage2bertini"][setting_name].value());
      break;
    case 1:
      setValue("PathFailureBertiniBase",setting_name,settings["stage2bertini"][setting_name].intvalue);
      break;
    case 2:
      setValue("PathFailureBertiniBase",setting_name,settings["stage2bertini"][setting_name].doubvalue);
      break;
    default:
      break;
    }
...
std::string ProgSettings::WriteConfigStepOne(){
  ...
  settingmap::iterator iter;
  for (iter=settings["stage1bertini"].begin(); iter != settings["stage1bertini"].end(); iter++){
    std::string setting_name = (*iter).first;
    config << setting_name << ": "
	   << settings["stage1bertini"][setting_name].value() << ";\n";
  }
...
std::string ProgSettings::WriteConfigStepTwo(){
  ...
  bool standardstep2;
  int sstep2 = this->settings["mode"]["standardstep2"].intvalue;
  if (sstep2 == 0){
    standardstep2 = false;
  }
  else{
  ...
  for (iter=settings["stage2bertini"].begin(); iter != settings["stage2bertini"].end(); iter++){
    std::string setting_name = (*iter).first;
    config << setting_name << ": "
	   << settings["stage2bertini"][setting_name].value() << ";\n";
  }
...
void ProgSettings::default_basic_bertini_values_stepone(){
  ...
  settings["stage1bertini"].clear();
  setValue("stage1bertini","TRACKTOLBEFOREEG",1e-5);
  setValue("stage1bertini","TRACKTOLDURINGEG",1e-6);
  setValue("stage1bertini","FINALTOL",1e-11);
  setValue("stage1bertini","IMAGTHRESHOLD",1e-8);
  setValue("stage1bertini","SECURITYLEVEL",0);
  setValue("stage1bertini","USERHOMOTOPY", 0);
  ProgSettings::save();
  return;
}
...
void ProgSettings::default_basic_bertini_values_steptwo(){
	...
	// these should all be the same as default bertini settings.
  settings["stage2bertini"].clear();
  setValue("stage2bertini","TRACKTOLBEFOREEG",1e-5);
  setValue("stage2bertini","TRACKTOLDURINGEG",1e-6);
  setValue("stage2bertini","FINALTOL",1e-11);
  setValue("stage2bertini","IMAGTHRESHOLD",1e-8);
  setValue("stage2bertini","SECURITYLEVEL",0);
  ProgSettings::save();
  return;
}
...
void ProgSettings::GetNumFilesTime(){
  ...
  if (valint==0){
    setValue("parallelism","numfilesatatime",400);
  }
  else{
...
void ProgSettings::GetWriteMCFileUserDef(){
  setValue("files","writemeshtomc",get_int_choice("would you like to produce a mc file during step2, when using computer-generated mesh?",0,1));
  return;
}
...
void ProgSettings::SolverModeMenu(){
  ...
  menu << "\n\nSolver Mode Settings:\n\n"
	<< "1) Standard stage2 run.\n"
	<< "2) File to use for stage2 start file\n"
	<< "*\n"
	<< "0) go back\n"
	<< "\n: ";
  while (choice!=0) {
...
void ProgSettings::StepOneMenu(){
  ...
  menu << "\n\nStage1 Settings:\n\n"
	...
	int choice = -1001;
	while (choice!=0) {
	  ProgSettings::DisplayCurrentSettings("stage1bertini");
	  ...
	  case 1:
	    ProgSettings::ChangeSetting("stage1bertini");
	    break;
	    
	  case 2:
	    ProgSettings::RemoveSetting("stage1bertini");
	    break;
	    
	  case 3:
	    ProgSettings::AddSetting("stage1bertini");
	    break;
...
void ProgSettings::StepTwoMenu(){
  ...
  int choice = -1001;
  while (choice!=0) {
    ProgSettings::DisplayCurrentSettings("stage2bertini");
    ...
    case 1:
      ProgSettings::ChangeSetting("stage2bertini");
      break;
      
    case 2:
      ProgSettings::RemoveSetting("stage2bertini");
      break;
      
    case 3:
      ProgSettings::AddSetting("stage2bertini");
      break;
